...
class IMGParser:
    ...
    def run(self):
        ...
        intialTracbarVals = [14,61,11,100] ##[2,53,0,100]
        #intialTracbarVals = [10,70,0,100]
        intialCarRef = 0
    
        ctrl_lat = purePursuit(lfd=0.8)  
        # image_parser = IMGParser()
        self.initializeTrackbars(intialTracbarVals, intialCarRef)
        src = self.valTrackbars()
        set_speed = self.valTrackbar_speed()
        
        self.perspectiveWarp_init(src)
        prevTime = curTime = time.time()
    
        cam_thread = Thread(target=self.get_image_continue)
        cam_thread.start()
    
        while not rospy.is_shutdown():
            curTime = time.time()
            
            '''ret, img_wlane = self.get_image()
            imgWarpPoints = img_wlane
            if not ret: continue'''
    
            if not self.saving_ret: continue
            self.saving_ret = False
            img_wlane =  self.saving_img
            # img_pixel=self.pixel_img
            imgWarpPoints = img_wlane
            ...
            img_wlane,pix_white = self.image_process(img_wlane)
            # img_pixel=self.pixel_image_process(img_pixel)
            
            # cv2.imshow("aaa", img_wlane)
            
            #region trackbar read
            src = self.valTrackbars()
            if self.trackbar_change_flag:
                self.trackbar_change_flag = False
                print("Recalculate perspectiveWarp.")
                self.perspectiveWarp_init(src)
            
            if self.speed_change_flag:
                self.speed_change_flag = False
                set_speed = self.valTrackbar_speed()
                print("Change the speed " + str(set_speed))
            #endregion
    
            birdView = self.perspectiveWarp(img_wlane, src)
    
            imgWarpPoints = self.drawPoints(imgWarpPoints, src)
            
            try:
                imgSliding, ploty, left_fit, right_fit, left_fitx, right_fitx = self.sliding_window(birdView, draw_windows=False)
                ...
                deviation, directionDev = self.offCenter(draw_info)
                ...
                ctrl_lat.pub_cmd(-1*1000, 0.5304)
            ...
            # cv2.imshow(self.trackbar_name, imgWarpPoints)
            ctrl_lat.pub_cmd(-1*1000, 0.5304)
            if pix_white < 1200.0:
                print("break")
                ctrl_lat.pub_cmd(0*1000, 0.5304)
                break
