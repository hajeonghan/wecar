...
class pure_pursuit:
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            if self.is_path==True and (self.is_odom==True or self.is_amcl==True):
                ...
                vehicle_position=self.current_positon
                rotated_point=Point()
                self.is_look_forward_point=False

                for num,i in enumerate(self.path_poses):
    ...
    def odom_callback(self, msg):
        ...
        self.current_positon.x=msg.pose.pose.position.x
        self.current_positon.y=msg.pose.pose.position.y
