...
class STOPLineEstimator:
    ...
    def get_x_pointer(self,lane_pts):
...
class purePursuit:
    ...
    def steering_angle(self,x_pred,y_pred_l,y_pred_r):
        ...
        if self.is_look_forward_point :
            steering_deg = math.atan2((2*self.vehicle_lenght*math.sin(theta)),self.lfd)*180/math.pi

            self.steering = np.clip(steering_deg,-22,22)/40 +0.5
            print(self.steering)
...
class CURVEFit:
    ...
    def preproces_pts(self,lane_pts):
        idx_list = []
    ...
    def fit_curve(self,lane_pts):
        x_left,y_left,x_right,y_right = self.preproces_pts(lane_pts)
